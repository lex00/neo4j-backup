...
def aggregate_one(group_alias: str) -> dict:
    ...
    head = store.latest_artifact_key(paths.alias_prefix(group_id, alias))
    if not head:
...
def verify_one(group_alias: str) -> dict:
    ...
    if not head:
        raise RuntimeError(f"no artifact for {group_id}/{alias}")
    physical = paths.physical_of_key(group_id, alias, head)
    src = paths.physical_prefix(group_id, alias, physical)
...
def prune_all() -> int:
    ...
    for g in pol.db_groups:
        ...
        for a in g.aliases:
            arts = store.list_artifacts(paths.alias_prefix(g.id, a))
            if not arts:
                continue
