...
| **Object-store backend** | S3 (boto3) | `CLOUD=azure` (Blob) / `gcp` (GCS). `object_store()` factory + per-cloud backends over one `ObjectStore` protocol (cloud-agnostic composites in `_BaseObjectStore`); validated on MinIO / Azurite / fake-gcs-server (#52) |
